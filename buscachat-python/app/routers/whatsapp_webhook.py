...
def _save_embedding_for_chat(chat_id: str, embedding: list[float] | None) -> None:
...
def _get_embedding_for_chat(chat_id: str) -> list[float] | None:
...
def _handle_menu(msg: dict, chat_id: str, text: str) -> dict:
    ...
    if text == "2":
        ...
        return _resp(chat_id, canal, "Escribe el *nombre completo* de la persona desaparecida:")
    if text == "3":
        return _resp(
            chat_id, canal,
...
def _handle_buscar_modo(msg: dict, chat_id: str, text: str) -> dict:
    ...
    if text == "2":
        ...
        return _resp(chat_id, canal, "Escribe el *nombre* de la persona que buscás:")
...
def _handle_reg_nombre(msg: dict, chat_id: str, text: str) -> dict:
    """Paso 1: nombre completo (obligatorio)."""
    canal = msg["canal"]
    if not text:
        return _resp(chat_id, canal, "Por favor escribe un nombre válido.")
    nombre = msg.get("text", "").strip()
    _set_state(chat_id, {"paso": "reg_edad", "nombre": nombre})
    return _resp(chat_id, canal, f"¿Qué edad tiene *{nombre}*? (respondé *omitir* si no sabés)")


def _handle_reg_edad(msg: dict, chat_id: str, text: str) -> dict:
    """Paso 2: edad (opcional, escribir 'omitir' para saltar)."""
    canal = msg["canal"]
    state = _get_state(chat_id)
    ...
    return _resp(chat_id, canal, f"¿Tenés el número de cédula de *{state['nombre']}*? (respondé *omitir* si no)")


def _handle_reg_cedula(msg: dict, chat_id: str, text: str) -> dict:
    """Paso 3: cédula (opcional)."""
    canal = msg["canal"]
    state = _get_state(chat_id)
...
def _handle_reg_ubicacion(msg: dict, chat_id: str, text: str) -> dict:
    """Paso 4: última ubicación conocida (escribir 'omitir' para saltar)."""
    canal = msg["canal"]
    state = _get_state(chat_id)
...
def _handle_reg_descripcion(msg: dict, chat_id: str, text: str) -> dict:
    """Paso 5: descripción física (opcional)."""
    canal = msg["canal"]
    state = _get_state(chat_id)
...
def _handle_reg_contacto(msg: dict, chat_id: str, text: str) -> dict:
    """Paso 7: contacto del reportante."""
    canal = msg["canal"]
    state = _get_state(chat_id)
    ...
    return _resp(chat_id, canal, resumen, buttons=[("si", "✅ Confirmar"), ("no", "❌ Cancelar")])
...
def _handle_reg_confirmar(msg: dict, chat_id: str, text: str) -> dict:
    ...
    canal = msg["canal"]
    state = _get_state(chat_id)
    _embedding = state.get("_embedding")  # capturar antes de limpiar
    _set_state(chat_id, None)
    ...
    return _resp(chat_id, canal, "Registro cancelado. Escribe *menu* para empezar de nuevo.")
...
def whatsapp_webhook(
    ...
    if accion == "buscar_por_foto":
        ...
        if match:
            return WebhookResponse(
                chat_id=chat_id,
                text=_format_person_info(match),
                accion=accion,
            )
        return WebhookResponse(
            chat_id=chat_id,
...
def _extract_meta_message(body: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if msg_type == "text":
        ...
        # Respuesta de botón: extraer el id como "texto"
    ...
    **POST**: mensaje entrante de WhatsApp.
    ...
    if msg["tipo"] == "imagen" and msg["imagen_ref"] and settings.meta_access_token:
        ...
        if image_bytes:
            ...
            # Guardar embedding en el estado para usarlo al confirmar registro
            _save_embedding_for_chat(msg["chat_id"], msg["_embedding"])
    ...
    if accion == "registrar_persona":
        ...
        log.info("REGISTER: msg._embedding=%s result._embedding=%s final=%s",
                 msg.get("_embedding") is not None,
                 result.get("_embedding") is not None,
                 embedding is not None)
        if embedding:
            report = _meta_register_with_embedding(
                session, settings, result, datos, chat_id, list(embedding)
            )
            log.info("REGISTER: saved report %s with embedding dims=%s", report.id, len(embedding))
        else:
            log.warning("REGISTER: no embedding found — saving without face data")
            report = bot_intake.register_missing_person(
    ...
    if accion == "buscar_por_foto":
        ...
        # Log: cuántos reportes con embedding hay en la DB
        from sqlmodel import select as _sel

        from app.models import BotReport as _BR
        total = session.exec(_sel(_BR).where(_BR.status == "missing")).all()
        with_emb = [r for r in total if r.face_embedding is not None]
        log.info("Photo search: %d missing reports, %d with embeddings", len(total), len(with_emb))
    ...
    if accion == "buscar_por_nombre":
        ...
        person = bot_intake.search_by_name(session, name) if name else None
        if person:
            # Buscar BotReport vinculado para info extra
            linked = _get_linked_bot_report(session, person.id)
            info = _format_person_info(linked) if linked else f"✅ *{person.full_name}*\nEstado: {person.status}"
            if person.cedula_masked:
                info += f"\nCédula: {person.cedula_masked}"
            if person.last_known_location:
                info += f"\nUbicación: {person.last_known_location}"
            ...
            "interactive": {
                ...
                "action": {
                    "buttons": [
                        ...
                        for bid, btitle in buttons[:3]  # WhatsApp limita a 3 botones
                    ]
                },
            },
        }
...
def _meta_register_with_embedding(
    ...
    Mapea todos los campos recolectados por el bot a ``MissingPerson`` y
    ``BotReport``.
    """
    import uuid
    ...
    person = MissingPerson(
        ...
        photo_url=result.get("imagen_ref"),  # URL o media ID de Meta
        source_date=utc_now(),
    )
    session.add(person)
    session.flush()
    ...
    *,
    ...
    if best_report.missing_person_id is not None:
        ...
        if person is not None:
            ...
            session.add(person)
            # Adjuntar al report para que _format_person_info los muestre
...
def _format_person_info(report: Any) -> str:
    ...
    # Ubicación: preferir la del BotReport, si no la del MissingPerson vinculado
    ubicacion = getattr(report, "location", None)
    if not ubicacion and report.missing_person_id:
        ubicacion = getattr(report, "_linked_location", None)
    if ubicacion:
    ...
    # Cédula desde MissingPerson
    if report.missing_person_id and getattr(report, "_linked_cedula", None):
