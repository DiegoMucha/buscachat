...
def handle_menu(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    if text in ("1", "buscar"):
...
def handle_buscar_modo(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    ...
    if text in SEARCH_QUERY_COMMANDS:
        ...
        return make_response(chat_id, canal, "Escribe el *numero de cédula* o *nombre* de la persona que buscas:")
    if text in SEARCH_PHOTO_COMMANDS:
        set_conversation_state(chat_id, {"paso": "buscar_modo_foto"}, store)
        return make_response(chat_id, canal, "¿Que tipo de busqueda por foto?", buttons=[("1", "Foto del rostro"), ("2", "Foto de cedula")])
    return make_response(chat_id, canal, "Elige *1* para buscar por cédula o nombre, o *2* para buscar por foto.",
                         buttons=[("1", "Buscar por cédula o nombre"), ("2", "Buscar por foto")])


def handle_buscar_modo_foto(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    if msg.get("imagen_ref"):
...
def handle_buscar_query(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    if not text:
...
def handle_buscar_foto(msg: dict[str, Any], chat_id: str, store: ConversationStateStore | None) -> dict[str, Any]:
    ...
    return {"chat_id": chat_id, "canal": msg["canal"], "source": msg.get("source"), "respuesta": None,
            "accion": "buscar_por_foto", "datos": {}, "imagen_ref": msg.get("imagen_ref"),
            "_embedding": msg.get("_embedding"), "sender": msg.get("sender"), "nombre": msg.get("nombre")}
...
def handle_buscar_ocr(msg: dict[str, Any], chat_id: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    image_bytes = msg.get("_image_bytes")
    if not image_bytes:
        return make_response(chat_id, canal, "Por favor envia una foto de la cedula." if not msg.get("imagen_ref") else "Procesando la imagen...")
    try:
        ...
        set_conversation_state(chat_id, None, store)
        return {"chat_id": chat_id, "canal": canal, "respuesta": None, "accion": "buscar_por_ocr",
...
def handle_reg_nombre(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    if not text:
    ...
    return make_response(chat_id, canal, f"¿Tenes una foto de la cedula de *{nombre}*? Enviala para rellenar los datos, o escribe *no* para continuar.")


def handle_reg_cedula_ocr(msg: dict[str, Any], chat_id: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    ...
    if msg.get("_image_bytes"):
        try:
            ...
            if ocr_data.get("nombre") or ocr_data.get("cedula"):
                ...
                set_conversation_state(chat_id, state, store)
                return make_response(chat_id, canal, f"*Datos detectados por OCR:*\n\n¿Es correcto? *{state['nombre']}*", buttons=[("si", "Si"), ("no", "No")])
        except ImportError:
            pass
        except Exception:
            pass
...
def handle_reg_ocr_confirmar(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
...
def handle_reg_edad(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
    state["edad"] = None if text in ("omitir", "no se", "no sé", "ns") else text
    state["paso"] = "reg_cedula"
    set_conversation_state(chat_id, state, store)
    return make_response(chat_id, canal, f"¿Tenés el numero de cédula de *{state['nombre']}*? Responde *omitir* si no.")


def handle_reg_cedula(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
...
def handle_reg_ubicacion(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
    state["ubicacion"] = None if text in ("omitir", "no se", "no sé", "ns") else (msg.get("text", "").strip() or None)
    state["paso"] = "reg_descripcion"
    set_conversation_state(chat_id, state, store)
    return make_response(chat_id, canal, f"¿Como podrias describir a *{state['nombre']}*? Escribe *omitir* para saltar.")


def handle_reg_descripcion(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
    state["descripcion"] = None if text in ("omitir", "no", "ns") else msg.get("text", "").strip()
    state["paso"] = "reg_foto"
    set_conversation_state(chat_id, state, store)
    return make_response(chat_id, canal, f"📸 Envia una *foto* de *{state['nombre']}* para ayudar a identificarla. Escribe *omitir* si no tenes.")


def handle_reg_foto(msg: dict[str, Any], chat_id: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
    text = msg.get("text", "").strip().lower()
    ...
    if msg.get("imagen_ref"):
        ...
        state["paso"] = "reg_contacto"
        set_conversation_state(chat_id, state, store)
        return make_response(chat_id, canal, f"✅ Foto recibida. ¿Como podemos contactar a quien reporta?")
    return make_response(chat_id, canal, "Por favor envia una foto o escribe *omitir*.")


def handle_reg_contacto(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
...
def handle_reg_confirmar(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
    canal = msg["canal"]
    state = get_conversation_state(chat_id, store)
    embedding = state.get("_embedding")
    set_conversation_state(chat_id, None, store)
    if text in CONFIRM_COMMANDS:
        return {"chat_id": chat_id, "canal": canal, "respuesta": None, "accion": "registrar_persona",
                "datos": {"nombre": state["nombre"], "edad": state.get("edad"), "cedula": state.get("cedula"),
                          "ubicacion": state.get("ubicacion"), "descripcion": state.get("descripcion"),
                          "contacto": state.get("contacto")},
                "_embedding": embedding, "imagen_ref": state.get("imagen_ref"),
                "sender": msg.get("sender"), "nombre": msg.get("nombre", state["nombre"])}
    return make_response(chat_id, canal, "Registro cancelado.", buttons=[("menu", "Menu principal")])
...
def _search_query_action(msg: dict[str, Any], chat_id: str, text: str, store: ConversationStateStore | None) -> dict[str, Any]:
